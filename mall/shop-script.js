  ...
  return `
    <div class="product-card-small" data-product-id="${product.product_id}">
      ...
      <div class="product-price">
        ${
          discountPercent > 0
            ? `<span class="discount">${Math.round(discountPercent)}%</span>`
            : ""
        }
    ...
    if (sectionType === "recommended") {
      ...
      if (!selectedPetId) {
        ...
        const container = document.querySelector(
          ".recommendation-section:first-of-type .product-grid-small"
        );
        if (container) {
          container.innerHTML = products
            .map((product) => createProductCardSmall(product))
            .join("");
      ...
      console.log(
        "추천 알고리즘 호출, selectedPetId:",
        selectedPetId,
        "productType:",
        currentProductType
      );
      const recommendedProducts = await SupabaseService.getRecommendedProducts(
        selectedPetId,
        currentProductType,
        6
      );
      ...
      if (container) {
        if (recommendedProducts && recommendedProducts.length > 0) {
          ...
          if (products && products.length > 0) {
            ...
            container.innerHTML =
              '<p style="text-align: center; color: #959595;">제품을 찾을 수 없습니다.</p>';
          }
        }
      } else {
      ...
      console.log(
        `카테고리 필터 적용: category_id=${currentCategoryId}, product_type=${currentCategoryProductType}`
      );
      // orderBy를 'default'로 하면 product_id 순서대로 정렬
      const products = await SupabaseService.getProducts(
        currentCategoryId,
        currentCategoryProductType,
        20,
        "default"
      );
      console.log(
        `제품 로드 완료 (${sectionType}), 개수:`,
        products?.length || 0,
        "제품:",
        products
      );
      ...
      // 컨테이너 찾기 (여러 방법 시도)
      let container = document.querySelector(
        ".recommendation-section:last-of-type .product-grid-large"
      );
      if (!container) {
        // 대체 방법: 모든 recommendation-section 중 마지막 찾기
        const sections = document.querySelectorAll(".recommendation-section");
        if (sections.length > 1) {
          container = sections[sections.length - 1].querySelector(
            ".product-grid-large"
          );
        }
      }
      ...
      const categoryName =
        Object.keys(categoryMap).find(
          (key) => categoryMap[key] === currentCategoryId
        ) || "종합관리";
