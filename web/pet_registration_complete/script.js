...
function createPetCard(petData, index) {
  const genderSymbol =
    petData.gender === "male" ? "♂" : petData.gender === "female" ? "♀" : "♂";
  ...
  // 건강 관심사 (없으면 "없음" 표시)
  const healthNames =
    petData.healthInterests && petData.healthInterests.length > 0
      ? petData.healthInterests
          .map((key) => healthMap[key])
  ...
  const hasImage =
    ...
    petData.profileImage !== null &&
    petData.profileImage !== undefined &&
    petData.profileImage.trim() !== "";

  const cardHtml = `
    <div class="pet-card" data-pet-index="${index}" data-pet-id="${
    petData.pet_id || ""
  }">
      <button class="edit-btn">수정</button>
      <button class="delete-btn" data-pet-id="${
        petData.pet_id || ""
      }">삭제</button>
      <div class="pet-info">
        <div class="pet-profile">
          <div class="pet-image" id="petImageContainer-${index}">
            ...
            ${
              hasImage
                ? `<img data-pet-index="${index}" src="${petData.profileImage}" style="display: block;" />`
                : ""
            }
          </div>
        </div>
        <div class="pet-details">
          ...
          <div class="pet-meta">
            ...
            <span class="pet-weight">${
              petData.weight
                ? petData.weight.replace(/kg/g, "").trim() + "kg"
                : ""
            }</span>
          </div>
          <div class="pet-tags">
  ...
  try {
    ...
    try {
      ...
      const filteredPetsData = petsData.filter(
        (p) => String(p.pet_id) !== String(petId)
      );
  ...
  petCards.forEach((petCard) => {
    ...
    petCard.addEventListener("click", (e) => {
      ...
      if (
        ...
        e.target.closest(".pet-profile") ||
        e.target.closest(".pet-image")
      ) {
        return;
      }
      ...
      editBtn.addEventListener("click", (e) => {
        ...
        try {
          ...
          if (
            !isNaN(parsedIndex) &&
            parsedIndex >= 0 &&
            parsedIndex < petsData.length &&
            petsData[parsedIndex]
          ) {
            // 기존 currentPetData를 깊은 복사로 복원 (수정 모드 표시를 위해)
            const petDataToEdit = JSON.parse(
              JSON.stringify(petsData[parsedIndex])
            );
    ...
    if (deleteBtn) {
      deleteBtn.addEventListener("click", (e) => {
        ...
        const petId =
          deleteBtn.getAttribute("data-pet-id") ||
          petCard.getAttribute("data-pet-id");

        if (!petId || petId === "") {
          alert("삭제할 반려동물 정보를 찾을 수 없습니다.");
          console.error(
            "pet_id를 찾을 수 없습니다. deleteBtn:",
            deleteBtn,
            "petCard:",
            petCard
          );
    ...
    const imageContainer = petCard.querySelector(
      `#petImageContainer-${petIndex}`
    );
    const imageInput = petCard.querySelector(
      `.petImageInput[data-pet-index="${petIndex}"]`
    );
  ...
  if (container && input) {
    ...
    input.addEventListener("change", (e) => {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          let petImage = container.querySelector(
            `img[data-pet-index="${petIndex}"]`
          );
    ...
    if (!pets || pets.length === 0) {
      ...
      if (petsContainer) {
        petsContainer.innerHTML =
          '<p style="text-align: center; color: #959595; padding: 20px;">등록된 반려동물이 없습니다.</p>';
      }
      return;
    }
  ...
  backBtn.addEventListener("click", () => {
    ...
    try {
      ...
      if (petsData.length > 0) {
        ...
        const currentPetData = JSON.parse(
          localStorage.getItem("currentPetData") || "{}"
        );
...
if (startBtn) {
  ...
  startBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      // 선택한 반려동물 카드가 있으면 그 반려동물 사용, 없으면 첫 번째 반려동물 사용
      let selectedPet = null;

      if (selectedPetCard) {
        // 선택한 카드의 pet_id 가져오기
        const selectedPetId = selectedPetCard.getAttribute("data-pet-id");
        if (selectedPetId) {
          // Supabase에서 해당 반려동물 정보 가져오기
          if (
            typeof SupabaseService !== "undefined" &&
            SupabaseService.getPetsByUserId
          ) {
            const pets = await SupabaseService.getPetsByUserId(userId);
            if (pets && pets.length > 0) {
              selectedPet = pets.find(
                (p) => String(p.pet_id) === String(selectedPetId)
              );
              console.log("선택한 반려동물 찾기:", {
                selectedPetId,
                selectedPet,
              });
            }
      ...
      if (!selectedPet) {
        if (
          ...
          if (pets && pets.length > 0) {
            ...
            console.log("기본 반려동물 선택:", selectedPet);
          }
        }
      }
      ...
      if (selectedPet) {
        ...
        console.log("선택된 반려동물 저장 완료:", selectedPet);
      }
    } catch (err) {
