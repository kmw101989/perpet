...
async function loadHospitalImage(hospitalName) {
  try {
    ...
    const matchedHospital = hospitals.find((hospital) => {
      return (
        hospital.hospital_name.includes(hospitalName) ||
    ...
    if (matchedHospital) {
      console.log(
        `병원 이미지 매칭 성공: ${hospitalName} → ${matchedHospital.hospital_name}`
      );
...
async function loadVetInfo() {
  ...
  if (vetInfoStr) {
    ...
    // 랜덤 이미지 생성 함수
    function getRandomImageUrl(type = "hospital") {
      const randomNum = Math.floor(Math.random() * 1000) + 1;
      return `https://picsum.photos/seed/${type}${randomNum}/200/200`;
    }
    ...
    if (hospitalImageUrl) {
      ...
      // 이미지가 없으면 랜덤 이미지 삽입
      const randomImageUrl = getRandomImageUrl("hospital");
      if (vetProfileImage) {
        vetProfileImage.style.backgroundImage = `url('${randomImageUrl}')`;
        vetProfileImage.style.backgroundSize = "cover";
        vetProfileImage.style.backgroundPosition = "center";
      }
      const randomAvatarUrl = getRandomImageUrl("vet");
      if (vetAvatarImage) {
        vetAvatarImage.style.backgroundImage = `url('${randomAvatarUrl}')`;
        vetAvatarImage.style.backgroundSize = "cover";
        vetAvatarImage.style.backgroundPosition = "center";
      }
    }
