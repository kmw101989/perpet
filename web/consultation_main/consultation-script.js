...
function createVetCard(vet) {
  const hashtagsHTML = vet.hashtags
  ...
  try {
    ...
    vetData.forEach((vet) => {
      ...
      const matchedHospital = allHospitals.find((hospital) => {
        ...
        return (
          hospital.hospital_name.includes(vet.hospital) ||
      ...
      if (matchedHospital) {
        ...
        console.log(
          `병원 이미지 매칭 성공: ${vet.hospital} → ${matchedHospital.hospital_name}`
        );
      } else {
...
function bindCardEvents() {
  ...
  vetCards.forEach((card) => {
    card.addEventListener("click", function (e) {
      ...
      if (
        !e.target.classList.contains("consult-button") &&
        !e.target.closest(".consult-button")
      ) {
