...
let agreeAllCheckbox, age14Checkbox, serviceTermsCheckbox, privacyTermsCheckbox, marketingTermsCheckbox;
let submitBtn;
  ...
  togglePasswordButtons.forEach((btn) => {
    btn.addEventListener("click", function (e) {
      ...
      if (targetInput.type === "password") {
        ...
        targetInput.style.fontFamily = '"JejuGothic", "Noto Sans KR", sans-serif';
...
function saveJoinMemberData() {
  try {
    // DOM 요소 확인
    if (!emailInput || !passwordInput || !passwordConfirmInput || !nicknameInput || 
        !genderInput || !residenceInput || !phoneInput || !marketingTermsCheckbox) {
...
function validateForm() {
  if (!emailInput || !passwordInput || !passwordConfirmInput || !nicknameInput || 
      !genderInput || !residenceInput || !phoneInput || !submitBtn) {
    return;
  }
...
function setupSubmitButton() {
  ...
  submitBtn.addEventListener("click", async function (e) {
    ...
    if (!submitBtn.disabled) {
      if (isFirstClick) {
        // 첫 번째 클릭: 필수 체크박스만 자동 선택 (marketingTerms 제외)
        termCheckboxes.forEach((checkbox) => {
          // marketingTerms (선택)광고성 정보 수집 전체 동의는 제외
          if (checkbox.id !== "marketingTerms") {
            checkbox.checked = true;
          }
        });
        
        // marketingTerms는 현재 상태 유지 (사용자가 선택할 수 있도록)
        // marketingTermsCheckbox는 자동 선택하지 않음
        
        // 필수 체크박스만 모두 체크되었으므로 agreeAllCheckbox 상태 업데이트
        updateAgreeAllCheckbox();

        // 체크박스 change 이벤트 트리거 (필수 체크박스 변경사항 반영)
        age14Checkbox.dispatchEvent(new Event("change", { bubbles: true }));
        serviceTermsCheckbox.dispatchEvent(new Event("change", { bubbles: true }));
        privacyTermsCheckbox.dispatchEvent(new Event("change", { bubbles: true }));
...
function restoreJoinMemberData() {
  try {
    ...
    if (!emailInput || !passwordInput || !passwordConfirmInput || !nicknameInput || 
        !genderInput || !residenceInput || !phoneInput || !marketingTermsCheckbox) {
      console.warn("DOM 요소들이 아직 초기화되지 않았습니다. 복원을 건너뜁니다.");
      return false;
    }
    ...
    if (joinMemberData.passwordConfirm !== undefined && passwordConfirmInput) {
      ...
      // password 타입일 때 스타일 적용
      if (passwordConfirmInput.type === "password" && passwordConfirmInput.value) {
    ...
    // 성별 복원
    if (joinMemberData.gender !== undefined && genderInput && genderButtons && genderButtons.length > 0) {
    ...
    // 거주지 복원
    if (joinMemberData.residence !== undefined && residenceInput && residenceButtons && residenceButtons.length > 0) {
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const isBackNavigation = referrer.includes("pet_registration") ||
