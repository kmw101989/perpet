...
function getSupabaseClient() {
  if (!createClient) {
    throw new Error('Supabase 클라이언트를 초기화할 수 없습니다. @supabase/supabase-js 패키지가 설치되어 있는지 확인하세요.');
  }
  
  const supabaseUrl = process.env.SUPABASE_URL || 'https://wdrirrlwmafmrqebpvxb.supabase.co';
  const supabaseKey = process.env.SUPABASE_ANON_KEY || 'sb_publishable_6TYMynQhG55NJ79kQdzQVA_DH1w8E2K';
...
async function getDiseaseIdsFromSymptoms(normalizedSymptoms) {
  ...
  try {
    ...
    // disease_id 중복 제거
    const diseaseIds = [...new Set(data.map(s => s.disease_id).filter(Boolean))];
    return diseaseIds;
  } catch (err) {
...
async function getRecommendedHospitals(categoryIds) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('hospitals')
      .select('hospital_id, hospital_name, address, rating, review_count, hospital_img')
...
async function getRecommendedProducts(categoryIds, targetSpecies = '강아지', productType = null) {
  const supabase = getSupabaseClient();
  ...
  try {
    ...
    console.log('[Chat Function] 입력 파라미터:', {
      ...
      categoryIdsType: Array.isArray(categoryIds) ? 'array' : typeof categoryIds,
      categoryIdsLength: Array.isArray(categoryIds) ? categoryIds.length : 'N/A',
      targetSpecies: targetSpecies,
      productType: productType
    });
    
    if (!Array.isArray(categoryIds) || categoryIds.length === 0) {
      console.error('[Chat Function] ❌ categoryIds가 유효하지 않음:', categoryIds);
      console.log('========================================');
      return [];
    }
    ...
    let query = supabase
      .from('products')
      .select('product_id, product_name, product_img, category, product_type, current_price, original_price, discount_percent')
    ...
    console.log('[Chat Function] ⏱️ 쿼리 실행 시간:', queryEndTime - queryStartTime, 'ms');
    ...
    if (error) {
      ...
      console.error('[Chat Function] 오류 상세:', JSON.stringify(error, null, 2));
      console.log('========================================');
      return [];
    }
    
    console.log('[Chat Function] ✅ 쿼리 실행 성공');
    console.log('[Chat Function] 📊 조회된 제품 개수:', data?.length || 0, '개');
    
    if (data && data.length > 0) {
            console.log('[Chat Function] 📦 조회된 제품 상세 정보:');
            data.slice(0, 5).forEach((p, idx) => {
              console.log(`[Chat Function]   [${idx + 1}] product_id: ${p.product_id}, name: ${p.product_name}, category: ${p.category}, product_type: ${p.product_type}`);
            });
      ...
      data.forEach(p => {
        ...
        productTypeCount[p.product_type] = (productTypeCount[p.product_type] || 0) + 1;
      });
      ...
      if (productType && categoryIds.length > 0) {
        ...
        console.log('[Chat Function] 재시도 조건: category IN', categoryIds, '(product_type 필터 제거)');
        ...
        console.log('[Chat Function] ⏱️ 재시도 쿼리 실행 시간:', retryEndTime - retryStartTime, 'ms');
        
        if (retryError) {
          console.error('[Chat Function] ❌ 재시도 쿼리 오류 발생');
          console.error('[Chat Function] 재시도 오류 코드:', retryError.code);
          console.error('[Chat Function] 재시도 오류 메시지:', retryError.message);
          console.error('[Chat Function] 재시도 오류 상세:', JSON.stringify(retryError, null, 2));
        } else {
          console.log('[Chat Function] ✅ 재시도 쿼리 실행 성공');
          console.log('[Chat Function] 📊 재시도 조회된 제품 개수:', retryData?.length || 0, '개');
          
          if (retryData && retryData.length > 0) {
            console.log('[Chat Function] 🎉 product_type 필터 제거 후 조회 성공!');
            ...
            retryData.forEach(p => {
              retryCategoryCount[p.category] = (retryCategoryCount[p.category] || 0) + 1;
              retryProductTypeCount[p.product_type] = (retryProductTypeCount[p.product_type] || 0) + 1;
            });
            console.log('[Chat Function] 📈 재시도 category 분포:', retryCategoryCount);
            console.log('[Chat Function] 📈 재시도 product_type 분포:', retryProductTypeCount);
            ...
            console.log('[Chat Function] 🔍 DB에 해당 category의 제품 존재 여부 확인 중...');
            ...
            if (checkError) {
              ...
              console.log('[Chat Function] 📊 확인 결과: category', categoryIds, '에 해당하는 제품', checkData?.length || 0, '개 존재');
            }
          }
        }
      } else {
        console.warn('[Chat Function] ⚠️ product_type 필터가 없어 재시도하지 않음');
      }
    }
    
    // 제품 정렬 및 제한 (target_species 컬럼이 없으므로 단순 정렬)
    console.log('[Chat Function] 🔄 정렬 및 제한 시작');
    console.log('[Chat Function] 정렬 전 제품 개수:', (data || []).length, '개');
    
    // 단순히 상위 3개만 선택 (target_species 필터링 제거)
    const filtered = (data || []).slice(0, 3);
    
    console.log('[Chat Function] ✅ 필터링 및 정렬 완료');
    console.log('[Chat Function] 📊 필터링 후 제품 개수:', filtered.length, '개');
    
    if (filtered.length > 0) {
      console.log('[Chat Function] 📦 최종 추천 제품 목록:');
      filtered.forEach((p, idx) => {
        console.log(`[Chat Function]   [${idx + 1}] product_id: ${p.product_id}, name: ${p.product_name}`);
      });
    }
    ...
    if (filtered.length === 0) {
      if (data && data.length > 0) {
        console.warn('[Chat Function] ⚠️ 제품이 조회되었지만 필터링 후 0개 - 모든 제품 포함하도록 수정');
        console.log('[Chat Function] 📦 필터링 없이 상위 3개 제품 반환');
        const fallbackProducts = data.slice(0, 3).map(p => ({
          product_id: p.product_id,
          product_name: p.product_name
        }));
        console.log('[Chat Function] ✅ 최종 반환 제품 개수:', fallbackProducts.length, '개');
        console.log('========================================');
    ...
    console.log('[Chat Function] ✅ 최종 반환 제품 개수:', finalProducts.length, '개');
    console.log('========================================');
...
function findDiseaseByKeyword(keyword, diseases) {
  ...
  if (match) {
    console.log('[Chat Function] 정확한 매칭으로 질병 발견:', match.disease_name);
    return match;
  }
  ...
  for (const [key, categoryId] of Object.entries(keywordToCategoryId)) {
    if (keywordNormalized.includes(key) || key.includes(keywordNormalized)) {
      ...
      if (match) {
        console.log('[Chat Function] 키워드 매핑으로 질병 발견:', key, '→', match.disease_name, '(category_id:', categoryId, ')');
        return match;
      }
    }
  }
  ...
  for (const [key, values] of Object.entries(keywordMap)) {
    if (values.some(v => keywordNormalized.includes(v) || v.includes(keywordNormalized))) {
      // 해당 키워드와 관련된 질병 찾기
      match = diseases.find(d => 
        d.disease_name.toLowerCase().includes(key) ||
        values.some(v => d.disease_name.toLowerCase().includes(v))
      );
      if (match) {
        console.log('[Chat Function] 키워드 맵으로 질병 발견:', match.disease_name);
        return match;
      }
    }
  }
...
async function analyzeSymptoms(userMessage, dbData, apiKey, history = []) {
  ...
  const symptomWords = [...new Set(symptoms.map(s => s.symptom_word).filter(Boolean))];
  ...
  // 증상 관련 키워드 확인
  const hasSymptomKeywords = symptomWords.some(symptom => 
  ...
  const systemPrompt = `너는 반려동물 건강 상담 보조 AI다.
...
판단 불가 시:
{
  ...
  for (const [key, categoryId] of Object.entries(keywordToCategoryId)) {
    ...
    if (key === '장') {
      ...
      shouldMatch = regex.test(messageForMatching) || messageForMatching.includes(key);
    }
    
    if (shouldMatch) {
      if (!directCategoryIds.includes(categoryId)) {
        directCategoryIds.push(categoryId);
        console.log('[Chat Function] 키워드로 category_id 직접 추출:', key, '→', categoryId);
      }
    }
  }
  ...
  if (hasRecommendationRequest && hasCategoryKeyword && directCategoryIds.length > 0 && !hasSymptomKeywords) {
    console.log('[Chat Function] 추천 요청 감지 - 질병 감지 없이 바로 추천');
    
    // 사용자 요청 분석
    const wantsProducts = userMessageLower.includes('제품') || userMessageLower.includes('상품') || userMessageLower.includes('사료') || userMessageLower.includes('영양제') || userMessageLower.includes('간식');
    const wantsHospitals = userMessageLower.includes('병원') || userMessageLower.includes('예약') || userMessageLower.includes('진료');
    ...
    if (wantsProducts && !wantsHospitals) {
      ...
      recommendedProducts = await getRecommendedProducts(directCategoryIds, '강아지', detectedProductType);
      console.log('[Chat Function] 직접 제품 추천 결과:', recommendedProducts.length, '개', { categoryIds: directCategoryIds, productType: detectedProductType });
    } else if (wantsHospitals && !wantsProducts) {
      // 병원만 추천
      recommendedHospitals = await getRecommendedHospitals(directCategoryIds);
      console.log('[Chat Function] 직접 병원 추천 결과:', recommendedHospitals.length, '개');
    } else {
      // 둘 다 추천 (제품 우선)
      recommendedProducts = await getRecommendedProducts(directCategoryIds, '강아지', detectedProductType);
      recommendedHospitals = await getRecommendedHospitals(directCategoryIds);
      console.log('[Chat Function] 직접 추천 결과:', { products: recommendedProducts.length, hospitals: recommendedHospitals.length, productType: detectedProductType });
    }
    
    // 메시지 생성
    let message = '';
    if (recommendedProducts.length > 0) {
      const categoryName = Object.entries(keywordToCategoryId).find(([k, v]) => directCategoryIds.includes(v))?.[0] || '관련';
      message = `${categoryName} 관련 ${detectedProductType || '제품'} 추천입니다. 아래 정보를 참고하세요.`;
      ...
      // 제품과 병원 모두 없을 때
      const categoryName = Object.entries(keywordToCategoryId).find(([k, v]) => directCategoryIds.includes(v))?.[0] || '관련';
      const productTypeText = detectedProductType ? `${detectedProductType} ` : '';
  ...
  try {
    ...
    try {
      ...
      analysisResult = {
        normalized_symptoms: [],
        suspected_diseases: [],
        message: '증상을 분석하는 중 오류가 발생했습니다. 증상을 다시 설명해주시면 도움을 드릴 수 있어요.'
      };
    }
    ...
    if (analysisResult.suspected_diseases && analysisResult.suspected_diseases.length > 0) {
    ...
    if (possibleDisease && validatedDiseases.length === 0) {
      const alreadyIncluded = validatedDiseases.some(d => d.disease_id === possibleDisease.disease_id);
      if (!alreadyIncluded) {
        console.log('[Chat Function] 키워드 매칭으로 질병 발견:', possibleDisease.disease_name);
    ...
    const isUncertain = validatedDiseases.length === 0 && validatedSymptoms.length === 0 && directCategoryIds.length === 0;
    ...
    if (!isUncertain) {
      ...
      if (validatedDiseases.length > 0) {
        ...
        if (diseaseIds.length > 0) {
          ...
          const diseasesWithCategories = await getDiseasesWithCategories(diseaseIds);
          
          // validatedDiseases와 매칭하여 category_id 추가
          finalDiseases = validatedDiseases.map(d => {
            const diseaseInfo = diseasesWithCategories.find(di => di.disease_id === d.disease_id);
            return {
              disease_id: d.disease_id,
          ...
          // category_ids 추출
          const diseaseCategoryIds = [...new Set(diseasesWithCategories.map(d => d.category_id).filter(Boolean))];
          
          console.log('[Chat Function] 질병 기반 추출된 category_ids:', diseaseCategoryIds);
          console.log('[Chat Function] 키워드 기반 directCategoryIds:', directCategoryIds);
          
          // 키워드 기반 category_id가 있으면 우선 사용, 없으면 질병 기반 사용
          if (directCategoryIds.length > 0) {
            categoryIds = directCategoryIds;
            console.log('[Chat Function] 키워드 기반 category_id 우선 사용:', categoryIds);
          } else {
            categoryIds = diseaseCategoryIds;
            console.log('[Chat Function] 질병 기반 category_id 사용:', categoryIds);
          }
          ...
          if (categoryIds.length > 0) {
            const userMessageLower = userMessage.toLowerCase();
            const wantsProducts = userMessageLower.includes('제품') || userMessageLower.includes('상품') || userMessageLower.includes('사료') || userMessageLower.includes('영양제') || userMessageLower.includes('추천해줘');
            const wantsHospitals = userMessageLower.includes('병원') || userMessageLower.includes('예약') || userMessageLower.includes('진료');
            
            console.log('[Chat Function] 사용자 요청 분석:', { wantsProducts, wantsHospitals, userMessage });
            ...
            if (wantsProducts && !wantsHospitals) {
              ...
              console.log('[Chat Function] 제품 추천 결과:', recommendedProducts.length, '개');
            } else if (wantsHospitals && !wantsProducts) {
              // 병원만 추천
              console.log('[Chat Function] 병원만 추천 시작');
              recommendedHospitals = await getRecommendedHospitals(categoryIds);
              console.log('[Chat Function] 병원 추천 결과:', recommendedHospitals.length, '개');
            } else {
              ...
              if (validatedSymptoms.length === 0 && validatedDiseases.length > 0) {
                ...
                recommendedProducts = await getRecommendedProducts(categoryIds);
                recommendedHospitals = await getRecommendedHospitals(categoryIds);
                console.log('[Chat Function] 추천 결과:', { products: recommendedProducts.length, hospitals: recommendedHospitals.length });
              } else {
                // 증상 기반인 경우 병원 우선
      ...
      if (directCategoryIds.length > 0) {
        console.log('[Chat Function] 키워드 기반 추천 시작:', { directCategoryIds, categoryIds, recommendedProducts: recommendedProducts.length, recommendedHospitals: recommendedHospitals.length });
        
        // categoryIds가 없으면 directCategoryIds 사용, 있으면 병합
        if (categoryIds.length === 0) {
          categoryIds = directCategoryIds;
          console.log('[Chat Function] 키워드 기반 category_id로 직접 추천:', directCategoryIds);
        } else {
          // 병합 (중복 제거)
        ...
        const keywordCategoryIds = directCategoryIds.length > 0 ? directCategoryIds : categoryIds;
        
        if (wantsProducts && !wantsHospitals) {
          // 제품만 추천
          if (recommendedProducts.length === 0) {
            console.log('[Chat Function] 키워드 기반 제품 추천 실행 (제품만):', keywordCategoryIds);
            recommendedProducts = await getRecommendedProducts(keywordCategoryIds);
            console.log('[Chat Function] 키워드 기반 제품 추천 결과:', recommendedProducts.length, '개', recommendedProducts);
          } else {
            console.log('[Chat Function] 이미 제품 추천이 있음, 키워드 기반 재시도:', keywordCategoryIds);
            // 이미 추천이 있지만 키워드 기반으로 다시 시도
            const keywordProducts = await getRecommendedProducts(keywordCategoryIds);
            if (keywordProducts.length > 0) {
              recommendedProducts = keywordProducts;
              console.log('[Chat Function] 키워드 기반 제품 추천 성공:', recommendedProducts.length, '개');
            }
          }
        } else if (wantsHospitals && !wantsProducts) {
          // 병원만 추천
          if (recommendedHospitals.length === 0) {
            console.log('[Chat Function] 키워드 기반 병원 추천 실행 (병원만):', keywordCategoryIds);
            recommendedHospitals = await getRecommendedHospitals(keywordCategoryIds);
            console.log('[Chat Function] 키워드 기반 병원 추천 결과:', recommendedHospitals.length, '개');
          }
        } else {
          // 둘 다 추천 (제품 우선, 없으면 병원)
          // 키워드 기반으로 제품 재시도
          if (recommendedProducts.length === 0 || (directCategoryIds.length > 0 && recommendedProducts.length === 0)) {
            console.log('[Chat Function] 키워드 기반 제품 추천 실행 (둘 다):', keywordCategoryIds);
            const keywordProducts = await getRecommendedProducts(keywordCategoryIds);
            if (keywordProducts.length > 0) {
              recommendedProducts = keywordProducts;
              console.log('[Chat Function] 키워드 기반 제품 추천 성공:', recommendedProducts.length, '개');
            } else {
              console.log('[Chat Function] 키워드 기반 제품 추천 실패 (제품 없음)');
            }
          }
          if (recommendedHospitals.length === 0) {
            console.log('[Chat Function] 키워드 기반 병원 추천 실행 (둘 다):', keywordCategoryIds);
            recommendedHospitals = await getRecommendedHospitals(keywordCategoryIds);
            console.log('[Chat Function] 키워드 기반 병원 추천 결과:', recommendedHospitals.length, '개');
          }
        }
    ...
    if (isUncertain) {
      return {
        ...
        message: analysisResult.message || '현재 정보만으로 특정 질병을 유추하기 어렵습니다. 증상을 조금 더 자세히 알려주시면 도움을 드릴 수 있어요.'
      };
    } else {
      // 추천이 없는 경우 AI 메시지 조정
      let finalMessage = analysisResult.message || '증상을 분석했습니다. 정확한 진단을 위해 병원 방문을 권장드립니다.';
      
      const userMessageLower = userMessage.toLowerCase();
      const wantsProducts = userMessageLower.includes('제품') || userMessageLower.includes('상품') || userMessageLower.includes('사료') || userMessageLower.includes('영양제');
      
      if (recommendedHospitals.length === 0 && recommendedProducts.length === 0) {
        if (wantsProducts) {
          finalMessage = '현재 등록된 제품 정보가 제한적이므로, 자사몰에서 관련 제품을 확인해보시거나 가까운 동물병원에 상담을 받아보시기 바랍니다.';
        } else {
          finalMessage = '현재 등록된 병원 정보가 제한적이므로, 가까운 동물병원 방문을 우선 권장드립니다.';
        }
      }
    ...
    return {
      ...
      message: err.message || '증상 분석 중 오류가 발생했습니다. 다시 시도해주세요.'
    };
  }
}
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'OK', message: 'AI Chat Function is alive' })
    };
  }
